...
int main() {
	...
	do {
		...
		switch (r) {
			case FP_ENROLL_COMPLETE:
				...
				snprintf(str, sizeof(str), "enrollcomplete.php|%d", fingerid);
				updategui(str);
				break;
			case FP_ENROLL_FAIL:
	...
	snprintf(str, sizeof(str), "../fingers/%d", fingerid);
	FILE *fp = fopen(str, "w");
	if(fp == NULL) {
		printf("Failed writing finger");
		return 0;
	}
	fwrite(buf, sizeof(unsigned char), buflen, fp);
	fclose(fp);
	free(buf);
